...
void calculate_electrons_per_pixel(Detector * det, Experiment * experiment){
  int i;
  ...
  if( det->electrons_per_pixel==NULL )det->electrons_per_pixel = malloc(sizeof(float)*det->nx*det->ny);
...
void calculate_real_detector_output(Detector * det, Experiment * experiment, int beamstop_radius){
  int i;
  ...
  if( det->real_output==NULL )det->real_output = malloc(sizeof(float)*(det->nx/det->binning)*(det->ny/det->binning));
  i = 0;
  for(x = 0;x<det->nx/det->binning;x++){
    for(y = 0;y<det->ny/det->binning;y++){
      det->real_output[i] = 0;
      int rx = det->nx/2 - x*det->binning;
      int ry = det->ny/2 - y*det->binning;
      if(rx*rx+ry*ry > beamstop_radius*beamstop_radius){
...
void calculate_noiseless_detector_output(Detector * det, Experiment * experiment){
  int i;
  double h_c = 1.98644521e-25; /* Planck's constant * the speed of light */
  double photon_energy = h_c/experiment->wavelength; /* photon energy in joules */
  float electrons_per_photon = photon_energy/det->electron_hole_production_energy;
