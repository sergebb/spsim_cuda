...
typedef struct{
  int natoms;
  int * atomic_number;
  /* this position should be normalized,
   meaning should all fall in the interval 0,1 */
  float * pos;
  /* this is the box dimensions for the position
   pos*box should give the physical positions */
  float box[3];
}Molecule;
...
typedef struct {
  ...
  */
  ...
  Detector * detector;
  short gpu_compute;
  short random_orientation;
  short uniform_rotation;
  short binary_output;
  unsigned int number_of_experiments;
  unsigned int beamstop_radius;
